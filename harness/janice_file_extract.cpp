...
int main(int argc, char ** argv) {
    ...
    if (argc < 4) {
        std::cerr << "Insufficient arguments, expected usage is: janice_file_extract sdk_path input_filename output_filename (nThreads) " << std::endl;
        return -1;
    }
    ...
    janice_file_extract(argv[2], argv[3], nThreads);
